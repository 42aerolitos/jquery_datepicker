...
module JqueryDatepicker
  module FormHelper
    ...
    def datepicker(object_name, method, options = {}, timepicker = false)
      ...
      html += javascript_tag("$(document).ready(function(){$('##{input_tag.get_name_and_id["id"]}').#{method}(#{dp_options.to_json})});")
      html.html_safe
    end
...
module JqueryDatepicker::FormBuilder
  def datepicker(method, options = {})
    options[:value] = options[:value].nil? ? @object.send(method) : options[:value]
